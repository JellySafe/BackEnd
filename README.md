...
2. **어댑터 계층 테스트 확대**: `test/persistence.smoke-spec.ts` 가 위험도 조회·대시보드 집계·
   ...
   다음 순번으로 적당하다.
3. **공개 조회 캐시**: `GET /public/beaches` 와 위험도 상세는 트래픽이 가장 몰리는데 데이터는
   30분에 한 번만 바뀐다. 짧은 TTL 캐시로 DB 부하를 크게 줄일 수 있다.
   **다만 지금 넣지 않았다** — 실측한 부하가 없기 때문이다. 근거 없이 넣은 캐시는 그 자체가
   신선도 사고의 원인이 되고, 이 저장소는 근거 없는 것을 넣지 않는 쪽을 택해 왔다(#23).
   `jellysafe_oldest_risk_score_age_seconds` 와 응답 시간을 먼저 본 다음 판단한다.

...
닿았다는 신호**다. 그 지점을 넘긴 부하는 더 많은 사람을 처리하는 데 쓰이지 않고 대기열에서
기다리게 만들 뿐이다. 실패가 0 인 것은 좋은 신호다(포화에서 죽지 않고 느려지기만 했다).
...
2. **어댑터 계층 테스트 확대**: `test/persistence.smoke-spec.ts` 가 위험도 조회·대시보드 집계·
   ...
   다음 순번으로 적당하다.
3. **공개 조회 캐시**: 이제 **근거가 생겼다.** 부하 실측에서 천장이 ≈570 req/s 로 나왔고,
   `GET /public/beaches` 가 가장 무겁고 가장 많이 불리는데 데이터는 30분에 한 번만 바뀐다.
   넣기 전에 **운영 사양에서 다시 재서** 병목이 DB 왕복인지 Node 단일 프로세스인지 갈라야 한다
   (둘이면 캐시 효과가 다르다).

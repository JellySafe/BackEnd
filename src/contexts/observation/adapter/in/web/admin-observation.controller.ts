...
export class AdminObservationController {
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '- `measurements` : 그 관측소들이 **실제로 주는 값**. 연결·신선도와 다른 문제다',
      '',
      '⚠️ 실제로 **해변 12곳 중 5곳이 관측 데이터를 받지 못해** 공개 화면에서 `unknown` 으로',
      '집계된다. 그 원인이 매핑인지 수집인지 가르는 것이 이 API 의 목적이다.',
      '',
      '⚠️ **세 번째 원인이 있다 — 관측소가 그 값을 아예 안 주는 것.** 파고부이는 유향·유속을',
      '관측하지 않아서, 거기 붙은 해변은 `CURRENT_INFLOW` 가 영원히 결측이고 신뢰도가',
      '`medium` 에서 멈춘다. 연결도 정상이고 `ageMinutes` 도 정상이라 기존 항목으로는',
      '구분되지 않았다. `measurements` 가 그걸 가른다 — `blockedFactors` 에 값이 있으면',
      '**기다려서 해결되지 않는다.**',
      '',

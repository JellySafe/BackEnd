...
export class GroundtruthConfig {
  ...
  /**
   * 좌표 없는 출현을 **시군구 일치**로 해변에 붙일지. 기본 켬.
   *
   * ── 왜 기본이 켬인가 ─────────────────────────────────────────────────────────────
   * 국립수산과학원 주간보고에는 **좌표가 아예 없다**. PDF 에 시군구 이름만 있어 파서가
   * lat/lng 을 항상 null 로 둔다. 그래서 이걸 끄면 실데이터로는 정답이 **0건**이고 정확도를
   * 아예 못 잰다 — 실제로 그 상태로 배포됐다가 발견했다.
   *
   * ── 끄면 무엇이 좋아지나 ─────────────────────────────────────────────────────────
   * 남는 정답이 전부 해변 단위가 되어 **해변별 정확도**를 잴 수 있다. 현장 관측(간편 링크)이
   * 충분히 쌓이면 그쪽이 더 나은 증거이므로, 그때 끄는 것이 맞다.
   *
   * ⚠️ 켜도 해변별 지표는 오염되지 않는다. 시군구 정답은 actual_granularity='region' 으로
   *    표시돼 해변별 집계에서 제외된다(countOutcomesByBeach). 전체 정확도에만 들어간다.
   */
  get useRegionOccurrences(): boolean {
    if (!this.useOccurrencesAsActual) return false;
    return (this.config.get<string>('GROUNDTRUTH_USE_REGION_OCCURRENCES') ?? 'true').trim() !== 'false';
  }
